...
export default function QuestionarioDEI() {
  ...
  const validar = () => {
    ...
    if (etapa === 7) {
      if (!form.lgpd_aceite) return 'É necessário autorizar o tratamento dos dados para continuar.';
    }
    return '';
  };
  ...
  const enviar = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('candidatos_dei').insert([payload]);
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        ...
        {etapa === 1 && (
          <>
            ...
            <div style={styles.radioGroup}>
              {['Empregado(a) com carteira assinada (CLT)', 'Trabalhando como autônomo(a) / PJ', 'Desempregado(a)'].map(op => (
        ...
        {etapa === 7 && (
          <>
            <div style={styles.titulo}>🔒 Autorização e Privacidade de Dados</div>
            <div style={styles.subtitulo}>Leia com atenção antes de confirmar.</div>

            <div style={styles.lgpd}>
              <strong>Autorização para Tratamento de Dados Pessoais — Lei nº 13.709/2018 (LGPD)</strong><br /><br />
              Ao prosseguir, você autoriza a <strong>Genthe Consultoria</strong> a coletar, armazenar e tratar os dados pessoais fornecidos neste formulário exclusivamente para fins de análise e participação no presente processo seletivo.<br /><br />
              Os dados serão compartilhados com a <strong>empresa contratante do processo seletivo</strong>, mantendo-se o sigilo da identidade da organização até a etapa de entrevista presencial.<br /><br />
              Suas informações <strong>não serão utilizadas para fins comerciais</strong>, cedidas a terceiros não relacionados a este processo, nem divulgadas publicamente.<br /><br />
              Você poderá solicitar a exclusão dos seus dados a qualquer momento pelo e-mail <strong>contato@genthe.com.br</strong>.<br /><br />
              A base legal para este tratamento é o <strong>legítimo interesse</strong> (art. 7º, IX da LGPD) no contexto de processo seletivo de trabalho.
            </div>

            <div
              style={{
                ...styles.checkItem(form.lgpd_aceite),
                marginTop: '20px',
                fontWeight: '600',
              }}
              onClick={() => set('lgpd_aceite', !form.lgpd_aceite)}
            >
              <span style={{ marginTop: '1px' }}>{form.lgpd_aceite ? '✅' : '⬜'}</span>
